...
lock_screen_box_primary = wibox({ visible = false, ontop = true, type = "splash", screen = screen.primary })
awful.placement.maximize(lock_screen_box_primary)
lock_screen_box_primary.bg = beautiful.transparent
lock_screen_box_primary.fg = beautiful.nord6

--- Add lockscreen to each screen
awful.screen.connect_for_each_screen(function(s)
    if s == screen.primary then
        s.mylockscreen = lock_screen_box_primary
    else
        lock_screen_box_other = wibox({ visible = false, ontop = true, type = "splash", screen = s })
        awful.placement.maximize(lock_screen_box_other)
        lock_screen_box_other.bg = beautiful.transparent
        lock_screen_box_other.fg = beautiful.nord6

        s.mylockscreen = lock_screen_box_other
        -- s.mylockscreen = helpers.ui.screen_mask(
        --     s,
        --     beautiful.lock_screen_bg or beautiful.exit_screen_bg or beautiful.nord1
        -- )
    end
end)
...
local function activate_word(w)
    for i, m in pairs(char_map[w]) do
        local text = m.text
        m.markup = helpers.ui.colorize_text(text, beautiful.nord6)
    end
end
